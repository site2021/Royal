<?php
    ...
    $consulta = "SELECT * FROM tblistaregistro
	...
	if($ok == true){
		...
		$titulosColumnas = array('No', 'COLEGIO','CODIGO', 'NOMBRE', 'GRADO', 'DIRECCION', 'BARRIO', 'COMUNA', 'CIUDAD', 'CELULAR',
                                 'NOMBRE ACUDIENTE','CEDULA','EMAIL', 'RECORRIDO', 'TARIFAV', 'RUTA', 'AM', 'PM', 'DOSDIR' );
		...
		while ($fila = mysqli_fetch_array($stmt)) {
			$objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('J'.$i,  $fila['celular1'].'/'.$fila['celular2'])
                    ->setCellValue('K'.$i,  $fila['nombreacudiente'])
                    ->setCellValue('L'.$i,  $fila['cedula'])
                    ->setCellValue('M'.$i,  $fila['email'])
                    ->setCellValue('N'.$i,  $fila['recorrido'])
                    ->setCellValue('O'.$i,  $fila['tarifav'])
                    ->setCellValue('P'.$i,  $fila['ruta'])
                    ->setCellValue('Q'.$i,  $fila['mrutaam'])
                    ->setCellValue('R'.$i,  $fila['mrutapm'])
                    ->setCellValue('S'.$i,  $fila['ruta2da']);
					$i++;
		...
		header('Content-Disposition: attachment;filename="contratosEstudiantes.xlsx"');
