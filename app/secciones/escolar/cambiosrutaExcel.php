<?php
    ...
    $consulta = "SELECT * FROM tbcambiosruta 
	...
	if($ok == true){
        ...
        $subtituloReporte = "Cambios de Ruta - ".$colegio;

		$titulosColumnas = array('No', 'FECHA CAMBIO', 'CODIGO', 'NOMBRE', 'ESTADO','RECORRIDO', 'RUTA COMPLETA', 'RUTA AM','RUTA PM','DOS DIRECCIONES', 'TARIFA', 'ESTADO', 'RUTA COMPLETA', 'RUTA AM','RUTA PM','DOS DIRECCIONES','TARIFA','DIAS RUTA1','DIAS RUTA2','PAGO RUTA1','PAGO RUTA2','DIFERENCIA');
		...
		$objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('G3',  "RUTA ANTERIOR")               
                    ->setCellValue('L3',  "RUTA VIGENTE")               
		...
		while ($fila = mysqli_fetch_array($stmt)) {
            ...
            $fechadia = new DateTime($fila['fechacambio']);
			...
			$objPHPExcel->setActiveSheetIndex(0)
        		    ...
        		    ->setCellValue('B'.$i,  $fila['fechacambio'])
		            ->setCellValue('C'.$i,  $fila['codigo'])
                    ->setCellValue('D'.$i,  $fila['nombre'])
                    ->setCellValue('E'.$i,  $fila['estadoant'])
                    ->setCellValue('F'.$i,  $fila['recorrido'])
                    ->setCellValue('G'.$i,  $fila['rutaant'])
                    ->setCellValue('H'.$i,  $fila['mrutaamant'])
                    ->setCellValue('I'.$i,  $fila['mrutapmant'])
                    ->setCellValue('J'.$i,  $fila['ruta2daant'])
                    ->setCellValue('K'.$i,  $fila['tarifavant'])
                    ->setCellValue('L'.$i,  $fila['estado'])
                    ->setCellValue('M'.$i,  $fila['ruta'])
                    ->setCellValue('N'.$i,  $fila['mrutaam'])
                    ->setCellValue('O'.$i,  $fila['mrutapm'])
                    ->setCellValue('P'.$i,  $fila['ruta2da'])
                    ->setCellValue('Q'.$i,  $fila['tarifav'])
                    // ->setCellValue('R'.$i,  $diasalida)
                    // ->setCellValue('S'.$i,  $formatomes)
                    ->setCellValue('R'.$i,  $ruta1)
                    ->setCellValue('S'.$i,  $ruta2)
                    ->setCellValue('T'.$i,  $pagoruta1)
                    ->setCellValue('U'.$i,  $pagoruta2)
                    ->setCellValue('V'.$i,  $saldopendiente);
					$i++;
        ...
        $objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion, "F3:K3");

        //OTRO CPOLOR NARANJA
        $objPHPExcel->getActiveSheet()->getStyle('L4:Q4')->applyFromArray($estiloTituloColumnas2);       
        $objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion2, "L5:Q".($i-1));       
        $objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion2, "L3:Q3");

        //OTRO COLOR VERDE CLARO
        $objPHPExcel->getActiveSheet()->getStyle('R4:V4')->applyFromArray($estiloTituloColumnas3);       
        $objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion3, "R5:V".($i-1));       
        // $objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion2, "L3:Q3");
