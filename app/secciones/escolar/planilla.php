...
function encabezado($xpdf,$wcual,$psector,$pnombreconductor,$pcelular,$pinterno,$pnombreauxiliar,$pcelularauxiliar,$pcantidad,
	...
	$xpdf->SetXY(250,55);$xpdf->Cell(5,5,'RE',1,0,'C');		
	$xpdf->SetXY(255,55);$xpdf->Cell(15,5,'DIAS',1,0,'C');		
...
function principal($pdf, $cual, $pcolegio, $pconexion, $pfecha){
	...
	$rs = mysqli_query($pconexion, "SELECT * FROM vst_listageneral V INNER JOIN tblistageneralnew l ON V.nombre = l.nombre WHERE (V.ruta='$cual' OR V.mrutaam='$cual' OR V.mrutapm='$cual' OR V.ruta2da='$cual') AND V.colegio = '$pcolegio' AND V.id_grado=l.grado AND V.estado=l.estado AND V.estado='A' AND V.codigo=l.codigo");
	...
	while($row = mysqli_fetch_object($rs)){
		...
		$pdf->SetXY(255,$fila);$pdf->Cell(15,5,$row->diasalternancia,1,0,'L');
		...
		if($row->recorrido==1){
			...
			$pdf->SetXY(250,$fila);$pdf->Cell(5,5,$xrecorrido,1,0,'L');
