...
function encabezado($xpdf,$wcual,$psector,$pnombreconductor,$pcelular,$pinterno,$pnombreauxiliar,$pcelularauxiliar,$pcantidad,
	...
	$coldesde = 160;
	while($coldesde<=265){
		$xpdf->SetXY($coldesde,55);$xpdf->Cell(5,5,'',1,0,'C');
		$xpdf->SetXY($coldesde+5,55);$xpdf->Cell(5,5,'',1,0,'C');
		$xpdf->SetXY($coldesde,60);$xpdf->Cell(5,5,'',1,0,'C');
		$xpdf->SetXY($coldesde+5,60);$xpdf->Cell(5,5,'',1,0,'C');
...
function principal($pdf, $cual, $pcolegio, $pconexion, $pfecha){
	...
	while($row = mysqli_fetch_object($rs)){
		...
		if($row->recorrido==1){
			...
			$coldesde = 160;
			while($coldesde<=265){
				$pdf->SetXY($coldesde,$fila);$pdf->Cell(5,5,'',1,0,'C');
