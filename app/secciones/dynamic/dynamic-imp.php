...
if(isset($_POST['action-import'])){
	...
	if(isset($_POST['itabla'])){
		...
		if(strtolower(end($chk_ext)) == "csv")
		{
		 ...
		 {
			// iniciar las cadenas de campos y valores con datos de la empresa
			// $campos = 'empresa_codigo';
			// $valores = "'".$empresa."'";

			$campos = '';
			$valores = '';

		 	for($i=0;$i<sizeof($equiva);$i++){
		 		//0-id, 1-tabla, 2-campo, 3-dato, 4-tipo, 5-descr, 6-etabla

		 		// todo esto lo vas a hacer si dato es diferente de ''
		 		$queDato = $equiva[$i][3];
		 		$campos = $campos.','.$equiva[$i][2];

		 		//adicionar valores segun equiva
		 		$queTipo = $equiva[$i][4];
		 		$queDescri = $equiva[$i][5];

		 		// validar si la descripcion esta vacia
		 		if($queDescri==''){
		 			// va a parte por el efecto de las comillas
		 			$valores = $valores.",'".trim($data[$queDato])."'";	

		 		}else {
		 			// validar el tipo de funcion
		 			// tipo: C-constante, 
		 			if($queTipo=='C'){
		 				// constante se asigna el valor de descri
		 				$nValor = "'".$queDescri."'";

		 			}elseif($queTipo=='F'){
			 			// ahora toca convertir la cadena segun descri por ejemplo
			 			$nValor = str_replace('x',$data[$queDato],$queDescri);

		 			}elseif($queTipo=='P'){
		 				// ejecutar funcion de PHP -- LISTA DE FUNCIONES DEFINIDAS --
		 				if($queDescri=='codigoMunicipio()'){	
		 					$nValor = "'".codigoMunicipio($data[$queDato])."'";

		 				}elseif($queDescri=='substr(codigoMunicipio())') {
							$nValor = "'".substr(codigoMunicipio($data[$queDato]),0,2)."'";

		 				}elseif($queDescri=='occurr(1)'){
		 					$nValor = "'".occurr($data[$queDato],1)."'";
		 				}elseif($queDescri=='occurr(2)'){
		 					$nValor = "'".occurr($data[$queDato],2)."'";
		 				}elseif($queDescri=='occurr(3)'){
		 					$nValor = "'".occurr($data[$queDato],3)."'";
		 				}elseif($queDescri=='occurr(4)'){
		 					$nValor = "'".occurr($data[$queDato],4)."'";
		 				}
		 			}

		 			$valores = $valores.",".$nValor;
...
<body>
	...
	<div class="botonera">
	    ...
	    <!-- DIV download excel //////////////////////////////////////////////////////////////// -->
	    <div id="div-download-excel" class="bordes bordes-left" style="visibility:hidden;">
	        <div class="tdiv">
	         	<a id="btnDownLoad" name="btnDownLoad" class="boton" onclick="exportarExcel()">
	...
	<div id="dlgImportar" class="easyui-dialog" closed="true" buttons="#" style="padding:2%">
		<form name="importaCSV" method="post" action="<?php echo $PHP_SELF; ?>" enctype="multipart/form-data">
			...
			<input id="cmbempresas" name="cmbempresas" class="easyui-combobox"
				   url="json/comboempresas.php" style="width:300px;display: none"
				   data-options="editable:false,valueField:'codigo',textField:'nombre'">
			</div>			
	...
	<script type="text/javascript">
		...
		function exportarExcel(){			
			location.assign("reporteexcel.php");	
		}
	</script>
</body>
</html>