...
$xcontrato = $_GET['contrato'];
$xcedularesponsable = $_GET['cedularesponsable'];

include '../../control/conex.php';

$rs = mysqli_query($conexion, "SELECT * FROM tbextractosrt WHERE extracto='$extracto' AND contrato='$xcontrato' AND cedularesponsable='$xcedularesponsable'");
...
// $xfechainicio =  htmlspecialchars(date("                   d                                       m                                       Y"));

// $xfechafin =  htmlspecialchars(date("                   d                                       m                                       Y"));
// $xfechainicio =  htmlspecialchars(date("m"));
...
$pdf->Image('img/movilidad_todos.jpg',12,18,80);
// escribir($pdf,107,15,97,28,'',0,0,'','','1');
//ISO
$pdf->Image('img/iso9001.jpg',104,18,34);

$pdf->Image('img/royaltour.jpg',145,13,62);

//NUMERO DE PLANILLA
// escribir($pdf,10,43,194,12,'',0,11,'C','B','1');
escribir($pdf,10,43,194,5,'FORMATO ÚNICO DE EXTRACTO DEL CONTRATO DEL SERVICIO PÚBLICO',0,11,'C','B','0');
escribir($pdf,10,47,194,5,'DE TRANSPORTE TERRESTRE AUTOMOTOR ESPECIAL',0,11,'C','B','0');

if($xfechainicio >= '2021-01-01'){
	escribir($pdf,10,51,194,5,'No. 366004318'.$xcontrato.$extracto,0,11,'C','B','0');
}
else{
	escribir($pdf,10,51,194,5,'No. 366004318',0,11,'C','B','0');
}
...
escribir($pdf,10,66,194,5,'RAZÓN SOCIAL DE LA EMPRESA DE TRANSPORTE ESPECIAL:',0,10,'L','B','0');
escribir($pdf,10,70,194,5,'ROYAL TOUR PLUS S.A.S',0,10,'l','','0');

//N° DE CONTRATO
escribir($pdf,10,77,194,3.5,'NIT:',0,10,'L','B','0');
escribir($pdf,20,77,194,3.5,'901.131.491-3',0,10,'L','','0');
...
// escribir($pdf,10,70,194,4.5,'',0,11,'C','B','0');
escribir($pdf,10,84,194,5,'CONTRATO No:',0,10,'L','B','0');
escribir($pdf,40,84,150,5,''.$xcontrato,0,10,'L','','0');

// //OBJETO DE CONTRATO
escribir($pdf,10,91,194,4,'CONTRATANTE:',0,10,'L','B','0');
escribir($pdf,10,95,194,5,''.$xcliente,0,10,'J','','0');

//ORIGEN
escribir($pdf,10,102,194,4,'NIT/CC: ',1,10,'J','B','0');
escribir($pdf,26,102,194,4,''.$xnit,1,10,'J','','0');

//ORIGEN
escribir($pdf,10,109,194,4,'OBJETO DE CONTRATO: ',1,10,'J','B','0');
escribir($pdf,10,113,194,4,''.$xobjetocontrato,1,10,'J','','0');


escribir($pdf,10,121,194,4,'ORIGEN-DESTINO: ',1,10,'J','B','0');
escribir($pdf,44,121,194,4,''.$xorigen.' - '.$xdestino,1,10,'J','','0');

escribir($pdf,10,128,194,4,'CONVENIO DE COLABORACIÓN: ',1,10,'J','B','0');
escribir($pdf,10,132,194,4,''.$xempresaafiliadora,1,10,'J','','0');


// escribir($pdf,10,78.4,194,2.9,''.$xrecorrido,1,9,'L','','0');
...
$contenido = 'http://intranet.cooperativaroyalexpress.com/app/secciones/fuecrt/planilla2020.php?extracto='.$extracto.'&contrato='.$xcontrato.'&cedularesponsable='.$xcedularesponsable; //Texto
...
$pdf->Image($dir.basename($filename),192,43.5,15,0,'PNG');

//TIPO
// escribir($pdf,10,157,194,5,''.$xmodalidad.'                      CON: '.$xempresaafiliadora,1,9,'L','','1');

//VIGENCIA
escribir($pdf,10,150,194,5,'VIGENCIA DEL CONTRATO',0,11,'C','B','0');

//FECHA INICIAL
escribir($pdf,10,157,78,6,'FECHA INICIAL',0,9,'L','','1');
// escribir($pdf,49,155,117,6,'DIA',0,5,'C','','0');
// escribir($pdf,87,155,117,6,'MES',0,5,'C','','0');
// escribir($pdf,127,155,117,6,'AÑO',0,5,'C','','0');
escribir($pdf,88,157,116,6,''.$xfechainicio,0,9,'C','','1');
// escribir($pdf,88,157,38,6,'',0,9,'C','','1');
// escribir($pdf,88,157,78,6,'',0,9,'C','','1');
//FECHA VENCIMIENTO
escribir($pdf,10,163,78,6,'FECHA VENCIMIENTO',0,9,'L','','1');

escribir($pdf,88,163,116,6,''.$xfechafin,0,9,'C','','1');
// escribir($pdf,88,163,38,6,'',0,9,'L','','1');
// escribir($pdf,88,163,78,6,'',0,9,'L','','1');

//CARACTERISTICAS DEL VEHICULO
escribir($pdf,10,175,194,5,'CARACTERÍSTICAS DEL VEHÍCULO',0,10,'C','B','0');
...
escribir($pdf,10,233,80,6,'CALLE 16 # 14-02 EDIFICIO 14 BIS OF 1A Teléfono: 3105451645',1,11,'L','','0');
escribir($pdf,10,246,90,6,'Email: servicioalcliente@royaltoursas.com',0,11,'L','','0');
...
$pdf->Image('img/firma.jpg',160,238,42);

$pdf->Image('img/sello.jpg',130,238,31);

escribir($pdf,128,230,40,10,'',0,0,'','','0');

escribir($pdf,9,254,72,5,'Documento firmado digitalmente mediante certificación custodiada por ANDES SCD S.A. según Ley 527 de 1999 y Decreto 2364 de 2012 y Resolución 6652 de 2019              1/1',0,7,'L','','0');

$pdf->Image('img/letra.JPG',205,182,1.7);

$pdf->Image('img/vigilado.jpg',205,232,7.5);
