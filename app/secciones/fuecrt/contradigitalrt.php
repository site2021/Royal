...
<body>
	...
	<div id="dlg" class="easyui-dialog" data-options="modal:true" style="width:800px;height:490px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
		<form id="fm" method="post" novalidate>
			...
			<div class="ftitle">VALOR SERVICIO</div>
				...
				<div class="fitem">
				<label>Fecha Firma CTO:</label>
				<input id="fechafirma" name="fechafirma" class="easyui-datebox" data-options="disabled:false,editable:true,formatter:myformatter,parser:myparser">
			</div>
		</form>
	</div>
	<div id="dlg-buttons">
	...
	<script type="text/javascript">
		...
		function newContraDigital(){
			...
			setVar('iniciocontrato',0,hoy());
			setVar('fincontrato',0,hoy());
			setVar('valorservicio',0,'0');
			setVar('fechafirma',0,hoy());
		...
		function aprobarContrato(){
			$.post('json/reg_maximo_contdigi.php', 
				function(result){
					if(result.success){
						var xcontra = $("#contrato").textbox('getValue');
						var xcliente = $("#cliente").textbox('getValue');
						if(xcontra ==''){
							if(xcontra == '' && xcliente !=''){
							...
							setVar('ncontrato',0,nnext);

							$("#contrato").textbox("setValue",nnext);
							// editContraDigital();
							// var url;
							url = 'json/contrato_save.php';
							$.messager.alert('Confirmación','Contrato guardado exitosamente, el número de contrato es: ' + nnext);
							...
							url = 'json/contradigital_update_cont.php?id='+row.id;
							alert( url);
							...
							$.messager.alert('Mensaje','el contrato debe ser guardado ');
						}
					}
						else{
							$.messager.alert('Mensaje','el contrato ya fue aprobado	');
						}
					}
					else{
		...
		function editContraDigital(){
			...
			if (row){
				...
				$('#fm').form('load',row);
				setVar('contrato',0,row.contrato);
