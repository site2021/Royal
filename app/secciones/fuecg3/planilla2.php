...
$rs = mysqli_query($conexion, "SELECT * FROM tbextractos WHERE extracto=$extracto");
...
// $link = 'http://intranet.cooperativaroyalexpress.com/app/secciones/fuec/planilla2.php?extracto='.$extracto;
...
escribir($pdf,10,58,194,5,'COOPERATIVA MULTIACTIVA DE TRANSPORTES ESPECIALES Y TURISMO ROYAL EXPRESS',0,9,'l','B','0');
escribir($pdf,160,55,44,5,'NIT',0,9,'J','','0');
escribir($pdf,160,58,44,5,'891.408.122-6',0,9,'J','B','0');

//N° DE CONTRATO
escribir($pdf,10,62,40,4,'CONTRATO No. '.$xcontrato,0,9,'L','','1');

//CONTRATANTE
escribir($pdf,50,62,154,4,'',0,11,'C','B','1');
escribir($pdf,50,62,194,5,'CONTRATANTE:',0,9,'L','','0');
escribir($pdf,76,62,150,5,''.$xresponsable,0,9,'L','','0');
escribir($pdf,160,62,44,5,'NIT/CC',0,9,'J','','0');
escribir($pdf,172,62,44,5,''.$xcedularesponsable,0,9,'J','','0');

// //OBJETO DE CONTRATO
escribir($pdf,10,66,194,4,'OBJETO CONTRATO',0,9,'L','','1');

// //TIPO TRANSPORTE
escribir($pdf,50,66,194,5,''.$xobjetocontrato,0,9,'J','','0');

//ORIGEN
escribir($pdf,10,70,194,4,'ORIGEN-DESTINO, DESCRIBIENDO RECORRIDO: '.$xorigen.' - '.$xdestino,1,9,'J','','1');

//DESCRIPCION
escribir($pdf,10,74,194,83,'',0,10,'C','','1');
escribir($pdf,10,74,194,5,''.$xrecorrido,1,9,'L','','0');
...
$contenido = 'http://intranet.cooperativaroyalexpress.com/app/secciones/fuec/planilla2.php?extracto='.$extracto; //Texto
