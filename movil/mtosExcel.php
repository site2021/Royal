<?php
    ...
    $consulta = "SELECT interno,fecha,taller,km,tipo,mecanico,descripcion FROM tbmantenimientos";
	...
	if($ok == true){
        ...
        $subtituloReporte = "Registro Mantenimientos ";

		$titulosColumnas = array('INT', 'FECHA', 'TALLER', 'KM', 'TIPO', 'MECANICO', 'DESCRIPCION');
		...
		while ($fila = mysqli_fetch_array($stmt)) {
			$objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('C'.$i,  $fila['taller'])
                    ->setCellValue('D'.$i,  $fila['km'])
                    ->setCellValue('E'.$i,  $fila['tipo'])
                    ->setCellValue('F'.$i,  $fila['mecanico'])
        ...
        $objPHPExcel->getActiveSheet()->getStyle('F5:G'.$i)->getNumberFormat()
        ->setFormatCode('#,###');
