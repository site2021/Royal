...
function irQuienesSomos(){
  $.mobile.go('#quienessomos');
}

function irPublicaciones(){
  $.mobile.go('#publicaciones');
}
...
function irReconocRuta(){
  var xhoy = hoy();
  setVar('rfecha',2,xhoy);

  $.mobile.go('#reconocruta');
}

function irCertificado(){
  $.mobile.go('#certificado');
}
...
function updateDgContr(){
  var xusuario = getVar('suser',0);
  $('#dgContrato').datagrid('load',{
    interno:xusuario
  }).datagrid('enableFilter');
}
...
function definirMenu(p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11){
  ...
  document.getElementById("mreconocruta").style.display=(p7==0?'none':'block');
  document.getElementById("mcertificado").style.display=(p8==0?'none':'block');
  document.getElementById("mquienessomos").style.display=(p9==0?'none':'block');
  document.getElementById("mpublicaciones").style.display=(p10==0?'none':'block');
  document.getElementById("mcontrato").style.display=(p11==0?'none':'block');
  // document.getElementById("mextractos").style.display=(p12==0?'none':'block');
}
